...
class Fairness_Metrics():
    ...
    def accuracy(self):
        return self.cm.weighted_average('ACC', none_omit=True)
    
    def precision(self):
        return self.cm.weighted_average('PPV', none_omit=True)

    def recall(self):
        return self.cm.weighted_average('TPR', none_omit=True)        

    def f1(self):
        return self.cm.weighted_average('F1', none_omit=True)
...
def load_weights(net, weights='imagenet', pth='./pretrained_weights/exp_0_final.pkl'):
    ...
    for name, _ in net.state_dict().items():
        if not ('ins' in name or 'running' in name or 'batches' in name or 'bn' in name or 'shortcut.1' in name or 'shortcut2' in name or 'conv1.1' in name):
